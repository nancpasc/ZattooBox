...
def build_root(addon_uri, addon_handle):
	content = [
		{'title': __addon__.getLocalizedString(30101), 'image': '', 'isFolder': True, 'url': addon_uri + '?' + urllib.urlencode({'mode': 'f', 'level': 'livetv_fav'})},
...
def build_recordingsList(addon_uri, addon_handle):
	resultData = _zapiSession.exec_zapiCall('/zapi/playlist', None)
...
def build_channelsList(addon_uri, addon_handle, category):
	zapiHelper = ZapiHelper(_zapiSession)
	channels = zapiHelper.get_channels(category)
	if channels is not None:
		content = []
		for record in channels:
			content.append({
				'title': record['title'], 'image': record['image'], 'isFolder': False,
...
def watch_recording(recording_id):
	...
	resultData = _zapiSession.exec_zapiCall('/zapi/watch', params)
	if resultData is not None:
...
data_folder = os.path.join(xbmc.translatePath(__addon__.getAddonInfo('path')).decode('utf-8'), 'resources', 'data', 'session')
_zapiSession = ZapiSession(data_folder)
if _zapiSession.init_session(__addon__.getSetting('username'), __addon__.getSetting('password')):
